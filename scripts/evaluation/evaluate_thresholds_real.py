...
def compute_metrics(y_true, y_pred):
    ...
    return {"tp": tp, "fp": fp, "tn": tn, "fn": fn, "precision": precision, "recall": recall, "f1": f1, "fpr": fpr, "accuracy": accuracy}
...
if __name__ == "__main__":
    ...
    baseline_files = [os.path.expanduser(f"~/iec104-lab/multi_runs_real/results/exp1_6_windows_run_{i}_features.csv") for i in range(1, 11)]
    replay_files = [os.path.expanduser(f"~/iec104-lab/multi_runs_real/results/replay/exp1_6_windows_run_{i}_features.csv") for i in range(1, 11)]
    print("=== THRESHOLD SWEEP: IEC-104 NOISY CONDITION (REAL DATA) ===")
    rows = []
    for threshold in thresholds:
        baseline_metrics = [evaluate_file(f, threshold, 0) for f in baseline_files]
        replay_metrics = [evaluate_file(f, threshold, 1) for f in replay_files]
        base_summary = summarize(baseline_metrics, ["fp", "tn", "fpr", "accuracy"])
        rep_summary = summarize(replay_metrics, ["tp", "fn", "precision", "recall", "f1"])
        row = {"threshold": threshold, "baseline_fpr": base_summary["fpr"][0], "baseline_fpr_std": base_summary["fpr"][1], "baseline_accuracy": base_summary["accuracy"][0], "replay_precision": rep_summary["precision"][0], "replay_recall": rep_summary["recall"][0], "replay_recall_std": rep_summary["recall"][1], "replay_f1": rep_summary["f1"][0]}
        rows.append(row)
        print(f"\nThreshold {threshold}")
    ...
    out_csv = os.path.expanduser("~/iec104-lab/multi_runs_real/results/iec104_threshold_sweep.csv")
